...
void clearStageBuffer(HANDLE s){
	...
	//Begin Buffer Clearing Fix from rzingel
		...
		//pText contains all info from the stage
		//printf(" [(clearStageBuffer) STAGE REPLY] `%s`\n",pText); // Display to console
        free(pText); 
 		return;
...
void setStartingStageLoc(HANDLE s){
	DWORD Length;
	char* buff=(char*) malloc(sizeof(char)*1024);
	sprintf(buff,"HERE X=0 Y=0\r");
...
CvPoint getStageLoc(HANDLE s){
	clearStageBuffer(s);
	DWORD Length;
	char* buff=(char*) malloc(sizeof(char)*1024);
	sprintf(buff,"WHERE X Y\r");
	SendCommandToStage(s, buff, strlen(buff), &Length, NULL);
	free(buff);
		_sleep(30);
		DWORD dwErrors; 
		...
		long px;
		long py;
		px = strtol(pText+3, &ptr, 10);
		printf(" [STAGE REPLY] \'%s\'",pText); // Display to console
		py = strtol(ptr+1, &ptr2,10);
		//printf(" Current stage pos: x=%ld, y=%ld\n",px,py); 
        free(pText); 
int sx = px;
int sy = py;
CvPoint spos;
