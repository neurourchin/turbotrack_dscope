...
int main (int argc, char** argv){
	...
	printf("exp->Params->DoCalib = %d \n",exp->Params->DoCalib);
	exp->Params->DoCalib = 0;
	printf("exp->Params->DoCalib = %d \n",exp->Params->DoCalib);
	...
	while (UserWantsToStop!=1) {
		...
		if (isFrameReady(exp)) {
			...
			int fps = CalculateAndPrintFrameRateAndInfo(exp);
			...
			/** Perform stage unit->pixel calibration **/
			printf("Now entering calibration routine -------------\n");
			printf("exp->Params->DoCalib = %d \n",exp->Params->DoCalib);
			//HandleCalibrationData(exp);
