...
/*
 * Sets up the WriteToDisk  given the base of a filename.
...
int AppendWormFrameToDisk(WormAnalysisData* Worm, WormAnalysisParam* Params, WriteOut* DataWriter){
	...
	cvStartWriteStruct(fs,NULL,CV_NODE_MAP,NULL);
		...
		/** Segmentation Info **/
		if(cvPointExists(Worm->stageLoc)){
		cvStartWriteStruct(fs,"StageLocation",CV_NODE_MAP,NULL);
			cvWriteInt(fs,"x",Worm->stageLoc->x);
			cvWriteInt(fs,"y",Worm->stageLoc->y);
		cvEndWriteStruct(fs);
		}

		// /** Illumination Information **/
		 cvWriteInt(fs,"DoCalib",Params->DoCalib);
